...
3. Review `.codex/platform/registry.yaml`. Its defaults support Codex workflows, subagents, and hooks plus shared skills in `.claude/skills`; change only the discovery paths that do not match the target project.
...
6. Add dependency IDs only after their target manifests exist. For a Hook, retain the actual registration in `.claude/hooks.json`.
