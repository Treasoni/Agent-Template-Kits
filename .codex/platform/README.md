...
`manifest.yaml` is the contract for every Workflow, Skill, Subagent, and Hook. The registry discovers manifests from the four configured directories in `registry.yaml`, validates their version, dependencies, and requested permissions, and checks that each Hook is registered in `.codex/hooks.json`.
...
The default registry discovers Codex workflows, subagents, and hooks alongside shared skills in `.agents/skills`. Adapt `registry.yaml` if the target project uses different directories. Keep each artifact’s `manifest.yaml` inside the directory configured for its kind. During migration, an agent or hook manifest may use a relative entrypoint such as `../outline-generator.md`; entrypoints may never leave the project root.
