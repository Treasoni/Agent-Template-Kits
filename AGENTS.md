...
| `.agents/skills/` | Gitignored Codex runtime adapter generated from canonical and locked sources. |
| `.codex/` | This repository's Codex rules, hooks, scripts, and workflows. |
| `.claude/skills/` | Gitignored Claude Code runtime adapter. |
| `.agent-sync/` | Optional gitignored multi-agent sync runtime. |
...
4. Run `python3 scripts/sync-runtime-skills.py --validate`; use `--apply` only to refresh ignored local runtime adapters.
...
- `.agents/skills/`, `.claude/skills/`, `.codebuddy/skills/`, `.agent-runtime/`, `.agent-sync/`, and host settings files are generated and gitignored.
- After a fresh clone, run `python3 scripts/sync-runtime-skills.py --apply`; add `--with-external` only for locked third-party development skills.
- Install `.agent-sync/` only when cross-profile config synchronization is needed, then run its `bootstrap.py --apply`.
...
- Shared skill sources live under `skills/`; local runtime adapters are generated with:
